...
export function VerifyEmailForm({ email, onBack, onSuccess }) {
      ...
      <div className="auth-body">
        <div className="verification-inputs">
          {Array.from({ length: 6 }).map((_, idx) => (
            <input key={idx} type="text" maxLength="1" className="verification-digit" inputMode="numeric" ref={el => (inputsRef.current[idx] = el)} onChange={e => onChange(idx, e.target.value)} onKeyDown={e => { if (e.key === 'Backspace' && !e.currentTarget.value && idx > 0) inputsRef.current[idx - 1].focus() }} />
          ))}
        </div>
